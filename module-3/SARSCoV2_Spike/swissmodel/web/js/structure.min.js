...
function detach(a){if(currentWin==window){if(0<$("#pv:visible",currentWin.document).length)currentWin=window.open(a.replace("xx","pv"),"_blank","resizable=1,status=1,toolbar=1,location=1,height=700,width=700"),main_pv=pv_viewer;else if(0<$("#ngl:visible",currentWin.document).length)main_ngl=ngl_stage,main_NGL_module=NGL,currentWin=window.open(a.replace("xx","ngl"),"_blank","resizable=1,status=1,toolbar=1,location=1,height=700,width=700");else return;$("#left-panel").is(".ui-resizable")&&$("#left-panel").resizable("destroy");
$("#left-panel").removeClass().addClass("detached");$("#right-panel").hide();$(window).trigger("resize")}}function attachPV(){pv_viewer=main_pv;attach()}function attachNGL(){var a=ngl_stage.viewerControls.getOrientation();NGL=main_NGL_module;ngl_stage=main_ngl;ngl_stage.removeAllComponents();showNGL(a);attach()}
function attach(){currentWin=window;var a=$("#right-panel").attr("class");if(a){a=a.split(/\s+/);for(var b=0;b<a.length;b++)size=a[b].split("-"),3==size.length&&$("#left-panel").addClass(size[0]+"-"+size[1]+"-"+(12-parseInt(size[2])))}$("#left-panel").removeClass("detached").resizable({handles:"e",stop:function(){}});$("#right-panel").show();$(window).trigger("resize");"undefined"!=typeof superposeTemplates&&superposeTemplates(!0);refreshBondsAndLigandHighlights()}
...
function highlightLigand(a,b,c){$(".ligandContact").removeClass("ligandContact");for(var d={},e=0;e<b.length;e++)b[e]&&"_"!=b[e].charAt(0)&&(res=b[e].substring(1),chain=b[e].charAt(0),d[chain]||(d[chain]=[]),d[chain].push(res));var f=$(".alignTbl tr[struc_id="+c+"]"),g=f[0].hasAttribute("keyseqchain")?"keyseqchain":"chain",h;for(h in d){var k=f.filter("["+g+"="+h+"]").find("span").toArray();for(e=0;e<d[h].length;e++)for(var l=0;l<k.length;l++)if(k[l].getAttribute("s")==d[h][e]){k[l].className="ligandContact";
...
function updatecols(){$(".alignTbl:first").alignment("viewerInitialised")}function fetchSecStruc(a){if(0<$("#pv:visible",currentWin.document).length){if(pv_viewer.get(a))return fetchSecStrucPV(a,pv_viewer.get(a).structure())}else if(0<$("#ngl:visible",currentWin.document).length&&getNGLComponentByName(a))return fetchSecStrucNGL(a,getNGLComponentByName(a).structure)}
function fetchBFactors(a,b,c){if(0<$("#pv:visible",currentWin.document).length){if(pv_viewer.get(a))return fetchBFactorsPV(a,pv_viewer.get(a).structure(),b,c)}else if(0<$("#ngl:visible",currentWin.document).length&&getNGLComponentByName(a))return fetchBFactorsNGL(a,getNGLComponentByName(a).structure,b,c)}
function setBFactorRange(a){var b=1E10,c=-1E10,d=0,e=0;for(chn in chainMapping.structures[a])for(res in chainMapping.structures[a][chn])if(chainMapping.structures[a][chn][res]&&chainMapping.structures[a][chn][res].bfactor){var f=chainMapping.structures[a][chn][res].bfactor;b=Math.min(f,b);c=Math.max(f,c);d+=f*f;e++}d=Math.sqrt(d/e);c=2*d;d=[];for(e=0;256>e;e++)d.push([e,e,255]);for(e=256;512>e;e++)d.push([255,512-e,512-e]);e=chainMapping.annotations.map(function(g){return g.key}).indexOf("bfactor_range");
chainMapping.annotations[e].rangeColours||(chainMapping.annotations[e].rangeColours={});chainMapping.annotations[e].rangeColours[a]={};for(chn in chainMapping.structures[a])for(res in chainMapping.annotations[e].rangeColours[a][chn]=[],chainMapping.structures[a][chn])chainMapping.structures[a][chn][res]&&chainMapping.structures[a][chn][res].bfactor&&(f=Math.round(512*(chainMapping.structures[a][chn][res].bfactor-b)/(c-b))-1,f=Math.max(f,0),f=Math.min(f,511),chainMapping.annotations[e].rangeColours[a][chn][res]=
d[f])}var ngl_stage=null,ngl_picked_atom=!1,ngl_picked_component=!1;
...
function centerResRangeNGL(a,b,c,d,e,f){var g="",h=getNGLComponentByName(d);if(h){h=h.structure;for(var k=0;k<a.length;k++)for(var l=f?b:c,n=b;n<=l;n++)e&&n!=b&&n!=l||isResidueInStructureNGL(h,a[k],n)&&(g+=n+":"+a[k]+" ");if(f)for(k=0;k<f.length;k++)isResidueInStructureNGL(h,f[k],c)&&(g+=c+":"+f[k]+" ");""!=g&&getNGLComponentByName(d).autoView(g,1E3)}}
...
d.forEach(function(g){scheme="line"==g?"element":"base"==g?"resname":"moleculetype";a.addRepresentation(g,{name:b+".dna",colorScheme:scheme,sele:"nucleic and not :_"})});e?ngl_stage.autoView():"function"==typeof on_viewer_ready_function?on_viewer_ready_function(b,a):a.autoView(1E3);updatecols(b);c&&refreshBondsAndLigandHighlights()}
function resAnnoLabel(a,b,c){var d="";if(chainMapping.structures[a]&&chainMapping.structures[a][b]&&chainMapping.structures[a][b][c]){chainMapping.chainMeta[a]&&chainMapping.chainMeta[a][b]&&(d+=" ("+chainMapping.chainMeta[a][b].display+")");for(var e=0;e<chainMapping.annotations.length;e++)if(chainMapping.annotations[e].active&&null!=chainMapping.structures[a][b][c][chainMapping.annotations[e].key]){var f=chainMapping.structures[a][b][c][chainMapping.annotations[e].key];d+="<br><small>"+chainMapping.annotations[e].label+
": "+("number"===typeof f?f.toFixed("ramachandran"==chainMapping.annotations[e].key?4:2):f);"qm"==chainMapping.annotations[e].key&&"confClass"==$.cookie("colourScheme")&&(1>f&&(f*=100),d=90<f?d+" (Very high)":70<f?d+" (Confident)":50<f?d+" (Low)":d+" (Very low)");d+="</small>"}}return d}
function fetchBFactorsNGL(a,b,c,d){var e=0,f=0;chainMapping.structures[a]||(chainMapping.structures[a]={});b.eachResidue(function(g){var h=g.chainname;if(!h.match(/(_|-)/)){var k=0;g.eachAtom(function(l){k+=l.bfactor});e+=k/g.atomCount;f++;coloursAssigned=!0;chainMapping.structures[a][h]||(chainMapping.structures[a][h]=[]);chainMapping.structures[a][h][g.resno]||(chainMapping.structures[a][h][g.resno]={});d&&!chainMapping.structures[a].invalidMapping&&residueProperties[chainMapping.structures[a][h][g.resno].resname].triplet!=
g.resname&&(chainMapping.structures[a].invalidMapping=!0);chainMapping.structures[a][h][g.resno][c]=k/g.atomCount}});"bfactor"==c&&setBFactorRange(a);return 0<e?e/f:0}
function fetchSecStrucNGL(a,b){var c;b.eachResidue(function(d){c=d.chainname;c.match(/(_|-)/)||(chainMapping.structures[a]||(chainMapping.structures[a]={}),chainMapping.structures[a][c]||(chainMapping.structures[a][c]=[]),chainMapping.structures[a][c][d.resno]||(chainMapping.structures[a][c][d.resno]={}),chainMapping.structures[a][c][d.resno].ss=d.sstruc.toUpperCase(),""==chainMapping.structures[a][c][d.resno].ss&&(chainMapping.structures[a][c][d.resno].ss="C"))});return!0}
function setColorForAtom(a,b,c){var d=a.structure().createEmptyView();d.addAtom(b);a.colorBy(pv.color.uniform(c),d)}var prevPicked=null;
...
prevPicked.color);if(null!==a){b=a.target();var c=b.qualifiedName().split(".");if("_"!==c[0].charAt(0)&&b.residue()._isAminoacid){var d=[0,0,0,0];a.node().getColorForAtom(b,d);prevPicked={atom:b,color:d,node:a.node()};setColorForAtom(a.node(),b,"red");d=a.object().geom.name();var e=b.residue().name(),f=b.residue().num();d=e+" "+f+" "+c[0]+resAnnoLabel(d,c[0],f);document.getElementById("pv_status").innerHTML=d;$("body").trigger("highlightResidue",{src:"structure",struc_id:a.object().geom.name(),chain:c[0],
resno:b.residue().num()})}else document.getElementById("pv_status").innerHTML=b.residue().name()+" "+b.residue().num()}else document.getElementById("pv_status").innerHTML="",prevPicked=null;pv_viewer.requestRedraw()}}});
...
color:schemeId});membraneData[a].viewTransform&&ngl_stage.viewerControls.spin([0,1,0],Math.PI);ngl_stage.autoView()})}}}
function fetchBFactorsPV(a,b,c,d){var e=0,f=0;b.eachChain(function(g){chn=g.name();chn.match(/(_|-)/)||(chainMapping.structures[a]||(chainMapping.structures[a]={}),chainMapping.structures[a][chn]||(chainMapping.structures[a][chn]=[]),g.eachResidue(function(h){var k=0,l=0;h.eachAtom(function(n){k+=n.tempFactor();l++});e+=k/l;f++;chainMapping.structures[a][chn][h.num()]||(chainMapping.structures[a][chn][h.num()]={});d&&!chainMapping.structures[a].invalidMapping&&residueProperties[chainMapping.structures[a][chn][h.num()].resname].triplet!=
h.name()&&(chainMapping.structures[a].invalidMapping=!0);chainMapping.structures[a][chn][h.num()][c]=k/l}))});"bfactor"==c&&setBFactorRange(a);return 0<e?e/f:0}
function fetchSecStrucPV(a,b){b.eachResidue(function(c){chn=c.chain().name();chn.match(/(_|-)/)||(chainMapping.structures[a]||(chainMapping.structures[a]={}),chainMapping.structures[a][chn]||(chainMapping.structures[a][chn]=[]),chainMapping.structures[a][chn][c.num()]||(chainMapping.structures[a][chn][c.num()]={}),chainMapping.structures[a][chn][c.num()].ss=c.ss())});return!0}function midpoint1d(a,b,c){return a+(b-a)*c}
function midpoint3d(a,b,c){return[midpoint1d(a[0],b[0],c),midpoint1d(a[1],b[1],c),midpoint1d(a[2],b[2],c)]}function showWaterPV(){pv_viewer.ballsAndSticks("water",pv_viewer.get("template").structure().select({rnames:["HOH"]}))}function showWaterNGL(){log.console("showWaterNGL not implemented")}function showWater(){0<$("#pv:visible",currentWin.document).length&&showWaterPV();0<$("#ngl:visible",currentWin.document).length&&showWaterNGL()}
function toggleBond(a,b){PLIP.AbstractViewer.isStructureEnabled(a)&&(plip_bonds.getBondById(a,b).toggle(),deHighlightBond(a,b,!0))}function highlightBond(a,b){PLIP.AbstractViewer.isStructureEnabled(a)&&plip_bonds.getBondById(a,b).highlight()}function deHighlightBond(a,b,c){PLIP.AbstractViewer.isStructureEnabled(a)&&(null==c&&(c=!0),plip_bonds.getBondById(a,b).dehighlight(!0,c))}
function highlightBonds(a,b){PLIP.AbstractViewer.isStructureEnabled(a)&&(plip_bonds.getBondsById(a,b).forEach(function(c){c.highlight(!1)}),plip_residues.redraw(a))}function deHighlightBonds(a,b){PLIP.AbstractViewer.isStructureEnabled(a)&&(plip_bonds.getBondsById(a,b).forEach(function(c){c.dehighlight(!1)}),plip_residues.redraw(a))}function clickBonds(a,b,c){toggleBonds(a,b,c);$("#plip_bonds_"+c).prop("checked")&&fitToBonds(a,b,c)}
function toggleBonds(a,b,c){a=$("#plip_bonds_"+c);b=a.prop("checked");a.prop("checked",!b);a.trigger("change")}function changeBondsCheckbox(a,b,c){var d=$("#plip_bonds_"+c),e=d.prop("checked");PLIP.AbstractViewer.isStructureEnabled(a)?(plip_bonds.getBondsById(a,b).forEach(function(f){e?f.show(!1):f.hide(!1);f.dehighlight(!1,!0)}),plip_residues.redraw(a),e&&plip_interactions_fit_bonds&&fitToBonds(a,b,c)):d.prop("checked",!1)}
function showBonds(a,b){PLIP.AbstractViewer.isStructureEnabled(a)&&(plip_bonds.getBondsById(a,b).forEach(function(c){c.show(!1);c.dehighlight(!1,!0)}),plip_residues.redraw(a))}function hideBonds(a,b){PLIP.AbstractViewer.isStructureEnabled(a)&&(plip_bonds.getBondsById(a,b).forEach(function(c){c.hide(!1);c.dehighlight(!1,!0)}),plip_residues.redraw(a))}function fitToBond(a,b){PLIP.AbstractViewer.isStructureEnabled(a)&&plip_bonds.getBondById(a,b).fitView()}
